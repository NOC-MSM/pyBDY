...
class Extract:
    def __init__(self, setup, DstCoord, Grid):
        ...
        nn_dist, self.nn_id = source_tree.query(
            dst_pts, k=4, eps=0, p=2, distance_upper_bound=0.5
        )
        ...
        dx = (
            lon[self.nn_id]
            - np.repeat(np.reshape(dst_lon, [dst_lon.size, 1]), 4, axis=1)
        ) * np.cos(
            np.repeat(np.reshape(dst_lat, [dst_lat.size, 1]), 4, axis=1) * np.pi / 180.0
        )
        dy = lat[self.nn_id] - np.repeat(
            np.reshape(dst_lat, [dst_lat.size, 1]), 4, axis=1
        )
        ...
        dist_wei = 1 / (
            np.divide(
                ...
                (
                    np.repeat(
                        np.reshape(np.nansum(dist_tot, axis=1), [dst_lat.size, 1]),
                        4,
                        axis=1,
                    )
                ),
        ...
        self.nn_wei = dist_wei / np.repeat(
            np.reshape(np.nansum(dist_wei, axis=1), [dst_lat.size, 1]), 4, axis=1
        )
        self.nn_wei[ind | self.msk] = 0.0
        ...
        if con in self.cons:
            ...
            if self.g_type != "t":
                ...
                if (
                    ...
                    self.harm_Im[con] = np.sum(
                        vIm[self.nn_id] * self.nn_wei, axis=1
                    ) / np.sum(self.bat[self.nn_id] * self.nn_wei, axis=1)
                    self.harm_Re[con] = np.sum(
                        vRe[self.nn_id] * self.nn_wei, axis=1
                    ) / np.sum(self.bat[self.nn_id] * self.nn_wei, axis=1)

                else:  # We convert to velocity using the regional model bathymetry
                    self.harm_Im[con] = (
                        np.sum(vIm[self.nn_id] * self.nn_wei, axis=1) / self.dst_dep
                    )
                    self.harm_Re[con] = (
                        np.sum(vRe[self.nn_id] * self.nn_wei, axis=1) / self.dst_dep
                    )

                # Rotate vectors

                self.harm_Im_rot[con] = self.rot_rep(
                    self.harm_Im[con],
                    self.harm_Im[con],
                    self.rot_str,
                ...
                self.harm_Re_rot[con] = self.rot_rep(
                    self.harm_Re[con],
                    self.harm_Re[con],
                    self.rot_str,
                    "en to %s" % self.rot_dir,
