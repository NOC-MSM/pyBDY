...
from pynemo.reader.ncml import NcMLFile, Reader, init_jnius
...
class Test(unittest.TestCase):
    ...
    def testVariable(self):
        ...
        self.assertAlmostEqual(
            dataset[0, 0, 0, 0], 18.945175, 6, "First value should be 18.9"
        )
        self.assertAlmostEqual(
            dataset[2, 10, 0, 0], 20.314891, 5, "2, 10, 0,0  value should be 18.9"
        )
    ...
    def testRepoMatching(self):
        ...
        self.assertAlmostEqual(
            repo["t"]["votemper"][0, 0, 0, 0],
            18.945175,
            6,
            "First value should be 18.9",
        )
        self.assertAlmostEqual(
            repo["t"]["votemper"][2, 10, 0, 0],
            20.314891,
            5,
    ...
    def testGridGroupTimeCounter(self):
        ...
        self.assertEqual(
            repo["t"].time_counter[0], 691416000, "The first time value doesn't match"
        )
        repo = Reader(testfile, 100)
        self.assertEqual(len(repo["t"].time_counter), 8, "Time counter should be 8")
        self.assertEqual(
            repo["t"].time_counter[0], 691416100, "The first time value doesn't match"
        )
