...
def gcoms_break_depth(bathy):
    ...
    # plt.show()
    hist_smooth = ndimage.uniform_filter(
        histbat.astype(float), int(nsmo) * 2 + 1, mode="nearest"
    )
...
def polcoms_select_domain(bathy, lat, lon, roi, dr):
    ...
    tmp[
        np.logical_and(
            np.logical_and(tmp != 0, np.logical_not(np.isnan(tmp))), tmp != -1
        )
    ] = 1
    ...
    for idx in range(0, len(lat_ob)):
        ...
        if i_e > len_lon - 2:
            ...
            i_e = i_0 + 2
        lat_slice = slice(
            max(lat_lon_index[0], 0), min(lat_lon_index[0] + 1 + 1, len_lat)
        )
        lon_slice = slice(
            max(lat_lon_index[1], 0), min(lat_lon_index[1] + 1 + 1, len_lon)
        )
        ...
        print(dx, dy, r)
        lat_slice = slice(
            max(lat_lon_index[0] - r, 0), min(lat_lon_index[0] + r + 1, len_lat)
        )
        lon_slice = slice(
            max(lat_lon_index[1] - r, 0), min(lat_lon_index[1] + r + 1, len_lon)
        )
        lat_pts = lat[lat_slice, lon_slice]
        lon_pts = lon[lat_slice, lon_slice]
        ...
        # dummy,dummy, distance_pts = geod.inv(len(lon_pts)
        #                                     * [lon_ob[idx]],len(lat_pts)*[lat_ob[idx]], lon_pts, lat_pts)
    ...
    for idx in range(0, len(lat_lb)):
        ...
        # geod = pyproj.Geod(ellps='WGS84')
        # dummy,dummy, distance_pts = geod.inv(len(lon_pts)
        #                                      *[lon_lb[idx]],len(lat_pts)*[lat_lb[idx]], lon_pts, lat_pts)
        # distance_pts=distance_pts/1000.0
