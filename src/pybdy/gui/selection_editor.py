...
class PolygonEditor(object):
    ...
    def __init__(self, axis, canvas):
        ...
        self._callback_ids.append(canvas.mpl_connect("draw_event", self.draw_callback))
    ...
    def refresh(self):
        ...
        if self.xy_values.shape[0] == 0:  # No points available
            ...
            if self.line is None:
                self.line = Line2D(
                    xval, yval, marker="o", markerfacecolor="r", animated=True
                )
            ...
            if self.polygon is None:
                self.polygon = Polygon(
                    self.xy_values, animated=True, alpha=polygon_alpha
                )
    ...
    def insert_datapoint(self, event):
        ...
        if self.xy_values.shape[0] <= 2:
            ...
            prev_d = dist_point_to_segment(
                event_point, self.xy_values[0], self.xy_values[-1]
            )
