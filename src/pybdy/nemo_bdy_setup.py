    ...
    def _get_var_name_value(self, line):
        ...
        if name_prefix == "ln":
            if value_str.find("true") != -1:
                ...
                raise ValueError(
                    "Cannot assign %s to %s, must be boolean" % (value_str, name)
                )
            ...
            for index, value in values.iteritems():
                ...
                for line in data:
                    ...
                    if line_without_comments == "":
                        continue
                    data_name, data_index, data_value = self._get_var_name_value(
                    ...
                    if data_name == name:
                        ...
                        if data_index == index:
                            if (
                                type(data_value).__name__ == "bool"
                                and type(value).__name__ != "bool"
                            ):
                                data[count] = _replace_var_value(
                                    line, data_value, self.bool_settings[name]
                                )
                                continue
...
def _trim(data):
    """
    Trim the sets of lines.

    Notes
    -----
    Removes empty lines/whitespaces and removes comments which start with !.
    """
    newdata = []
    while data:
        line = data.pop(0)
...
def _get_val(vars_dictionary, bool_vars_dictionary, line):
    ...
    Notes
    -----
    This function transverses input string and appends the setting name and its value to dictionary
    of settings and also if the setting name holds a boolean value then to the dictionary
    of boolean variables.
    This function also checks the type and raises error for ambiguous values.
    """
...
def _replace_var_value(original_line, value, new_value):
    """Replace the variable name value with new_value in the original_line."""
...
def _get_var_name(line):
    ...
    Notes
    -----
    Parse the line to find the name and if it is part of the array '()'
    then return name of variable and index of the array. if variable is not
