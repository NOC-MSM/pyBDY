...
from pynemo.nemo_bdy_setup import Setup
...
class Test(unittest.TestCase):
    ...
    def testReadingEmptyFile(self):
        ...
        self.assertEqual(
            setup.settings,
            {},
            "There are some default settings after reading empty file",
        )
        # delete empty file
        os.remove("test.bdy")
    ...
    def testEntryWithoutSpaceOnEitherSideOfEquals(self):
        ...
        setup = Setup("test.bdy")
        self.assertEqual(
            setup.settings, {"nonambigious": True}, "Didn't recognize valid setting"
        )
    ...
    def testEntryDifferentTypeEntries(self):
        ...
        self.assertEqual(
            setup.settings["nonambigious"], False, "Didn't recognize logical setting"
        )
        self.assertEqual(
            setup.settings["floatval"], 10.9, "Didn't recognize rn value in setting"
        )
        self.assertEqual(
            setup.settings["floatval2"], 20.9, "Didn't recongnize nn value in setting"
        )
        self.assertEqual(
