...
def _generate_grid(
    ...
    if degrees != 0:
        ...
        coords = np.stack((ds.glamt.values.ravel(), ds.gphit.values.ravel()),axis=1)
        lon, lat = _rotate(ds['glamt'].values.copy(), ds['gphit'].values.copy(),origin=origin,degrees=degrees)
        ds["glamt"] = ds["glamt"].copy(data=lon)
        ds["gphit"] = ds["gphit"].copy(data=lat)
        
        coords = np.stack((ds.glamt.values.ravel(), ds.gphit.values.ravel()),axis=1)
        lon, lat = _rotate(ds['glamu'].values.copy(), ds['gphiu'].values.copy(),origin=origin,degrees=degrees)
        ds["glamu"] = ds["glamu"].copy(data=lon)
        ds["gphiu"] = ds["gphiu"].copy(data=lat)
        
        coords = np.stack((ds.glamt.values.ravel(), ds.gphit.values.ravel()),axis=1)
        lon, lat = _rotate(ds['glamv'].values.copy(), ds['gphiv'].values.copy(),origin=origin,degrees=degrees)
        ds["glamv"] = ds["glamv"].copy(data=lon)
        ds["gphiv"] = ds["gphiv"].copy(data=lat)
        
        coords = np.stack((ds.glamt.values.ravel(), ds.gphit.values.ravel()),axis=1)
        lon, lat = _rotate(ds['glamf'].values.copy(), ds['gphif'].values.copy(),origin=origin,degrees=degrees)
...
def _add_mbathy(ds: Dataset) -> Dataset:
    ...
    Dataset
    """
    ds["mbathy"] = xr.where(ds["Bathymetry"] > 0, ds.z.size, 0)  # TODO: should this be bool?
    return ds
...
def generate_variables(
    ...
    # Define variables
    temp = xr.full_like(ds_domcfg.e3t, 20., dtype=np.double)
    salt = xr.full_like(ds_domcfg.e3t, 35., dtype=np.double)
    uvel = xr.full_like(ds_domcfg.e3t, 1. , dtype=np.double)
    vvel = xr.full_like(ds_domcfg.e3t, 1. , dtype=np.double)
    ssh  = xr.full_like(ds_domcfg.e2t, 0. , dtype=np.double)
       
    # Add attributes
    temp.attrs = dict(units="C", long_name=f"Temperature (degrees centigrade)")
    salt.attrs = dict(units="", long_name=f"Salinity (unitless)")
    uvel.attrs = dict(units="m/s", long_name=f"Zonal Velocity (m/s)")
    vvel.attrs = dict(units="m/s", long_name=f"Meridional Velocity (m/s)")
    ssh.attrs  = dict(units="m", long_name=f"Sea Surface Height (m)")
