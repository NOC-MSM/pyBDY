...
So for FES the only model currently supported only the location of the BDY file and thresholds
(if different from defaults) need to be provided.
For TPXO this would vary based on the resolution e.g. TPXO7.2 is 1/4 and 'tpxo'
...
File locations e.g. model reference location etc are all taken from bdy file that is passed
to the main function

To do this the script compiles a list of PyNEMO boundary amplitudes and phases and lat/lon's,
finds the closest value in the reference model (currently only FES is supported),
and then compares them.
If the absolute difference is greater than defined threshold then the location and parameter
(either Amp or Phase) is returned within a Pandas Dataframe which is then written to a spreadsheet.

Notes
-----
The script checks the Amplitude and Phase independently, so lat/lons for each are also returned.
Each HC is saved to a separate sheet in the spreadsheet.
The name of the spreadsheet contains meta data showing thresholds and reference model used.
Units for threshold are meters and degrees.

Update: fill values for FES are commonly returned at coastlines, this is due to the
nearest FES cell being land but PyNEMO will have interpolated data from the water.
In instance the code checks the cells aroud the fill value and averages both amplitude and phase
values (using HsinG,HcosG) to act as a reference.

Phase threshold is not longer required as it is applied using an function that references amplitude,
the idea is that the threshold is low for high amplitudes,
 e.g. 5 degrees for 1.0m and high for low amplitudes 80 degrees for 0.01 m.

Amplitudes at phase exceedance locataions are also returned to allow assessment of the impact,
e.g. low amplitude low impact
...
def main(bdy_file="inputs/namelist_local.bdy", model="fes"):
    ...
    # TODO maybe define Z and/or UV in bdy file? at the moment Z, U and Vs
    # are generated with no option for Z only.
    grids = ["Z"]  # ,'U','V']
    if model == "fes":
        logger.info("using FES as reference.......")
        # open writer object to write pandas dataframes to spreadsheet
        writer = pd.ExcelWriter(
            settings["dst_dir"] + "comparision_with_" + str(model) + ".xlsx",
            engine="xlsxwriter",
        )
        for key in constituents:
            for j in range(len(grids)):
                ...
                if error_log.empty is True:
                    logger.info(
                        "output file does not exceed threshold when compared \
with reference model..... thats good!"
                    )
                # if dataframe has values then these exceed the threshold,
                # log and save to excel spreadsheet using dataset
                # name e.g. M2Z (based on HC and grid) as name for the sheet
                if error_log.empty is False:
                    logger.warning(
...
def extract_PyNEMO_output(out_fname, grid):
    ...
    # read FES netcdf file, convert lon to -180 to 180,
    # rather than 0-360 it also converts amplitude from CM to M
    # return a dict
...
def read_fes(fes_fname, grid):
    ...
    # subset FES dict from read_FES,
    # this uses find_nearest to find nearest FES point using PyNEMO dict from extract_PyNEMO
    ...
    with warnings.catch_warnings():
        ...
        for i in range(np.shape(amp_sub)[1]):
            ...
            if amp_sub[0, i] == 184467436613926912.0000:
                logger.warning(
                    "found fill value in FES subset, taking nanmean from surrounding amplitude points"
                )
        ...
        for i in range(np.shape(phase_sub)[1]):
            ...
            if phase_sub[0, i] == 18446744073709551616.0000:
                logger.warning(
                    "found fill value in FES subset, taking nanmean from surrounding phase points"
                )
                # if there are fill values surrounding subset fill value change these to NaN
                if (
                    reference["phase"][idx_lat[0, i] + 1, idx_lon[0, i]]
                    == 18446744073709551616.0000
                ):
                    reference["phase"][idx_lat[0, i] + 1, idx_lon[0, i]] = np.nan
                if (
                    reference["phase"][idx_lat[0, i], idx_lon[0, i] + 1]
                    == 18446744073709551616.0000
                ):
                ...
                if (
                    ...
                    reference["phase"][idx_lat[0, i], idx_lon[0, i] - 1] = np.nan
                if (
                    reference["phase"][idx_lat[0, i] + 1, idx_lon[0, i] + 1]
                    == 18446744073709551616.0000
                ):
...
def compare_tides(pynemo_out, subset):
    ...
    with warnings.catch_warnings():
        ...
        logger.info(
            "percentage amplitude exceedance calculated using the following....."
        )
        ...
        pynemo_out["phase"][pynemo_out["phase"] < 0.0] = (
            pynemo_out["phase"][pynemo_out["phase"] < 0.0] + 360.0
        )
        subset["phase"][subset["phase"] < 0.0] = (
            subset["phase"][subset["phase"] < 0.0] + 360.0
        )
        ...
        err_log = pd.DataFrame(
            {
                "amp_lat": err_amp_lats,
