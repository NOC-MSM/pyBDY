...
from pybdy.profiler import process_bdy
...
def _create_bathymetry(
    ...
    if np.abs(degrees_parent) > 1e-8 or np.abs(degrees_child) > 1e-8:
        rotation = True
    else:
        rotation = False

    # Generate variables to use as source data
    ds_var_parent = generate_variables(ds_domcfg_parent, rotation=rotation)
...
def test_child_grid_colocated(handle_temp_dir) -> None:
    ...
    # Note that nbit and nbjt indexing start at 1
    nbit = ds_coords_bdy.nbit.values.squeeze()
    nbjt = ds_coords_bdy.nbjt.values.squeeze()
    votemper_parent_bdy = votemper_parent_sub[:, :, nbit - 1, nbjt - 1].values
    vosaline_parent_bdy = vosaline_parent_sub[:, :, nbit - 1, nbjt - 1].values

    assert np.min(nbit) == 2
    assert np.min(nbjt) == 2
    assert np.max(nbit) == int(ds_var_child.dims["x"]) - 1
    assert np.max(nbjt) == int(ds_var_child.dims["x"]) - 1

    # one_dim_idx = np.ravel_multi_index(np.transpose([*zip(nbit - 1, nbjt - 1)]),
    #  vosaline_parent_bdy.shape[2:]))

    # Reshape the bdy of the subsampled parent grid
    votemper_parent_bdy = votemper_parent_bdy.reshape(
        votemper_parent_bdy.shape[:-2] + (-1,)
    )
    vosaline_parent_bdy = vosaline_parent_bdy.reshape(
        vosaline_parent_bdy.shape[:-2] + (-1,)
    )

    # Extract only the correct combination of nbit and nbjt indices
    # Note that nbit and nbjt must be passed with indexes starting at 1
    one_dim_idx = sub2ind(nbit.shape, nbit - 1, nbjt - 1)
...
@pytest.mark.parametrize("handle_temp_dir", ["temp_test_002"], indirect=True)
def test_child_grid_rotated(handle_temp_dir) -> None:
    """Test the case where the child grid is rotated 45 degrees relatively to the parent grid."""
    # Define lat and lon boundaries of parent and child grids
    dlon, dlat = (1.0, 1.0)
    lon_i_offset, lon_f_offset, lat_i_offset, lat_f_offset = (10, -10, 5, -5)
    lon_i_parent, lon_f_parent, lat_i_parent, lat_f_parent = (-20.0, 10.0, 45.0, 65.0)
    ...
    depth_i, depth_f, depth_thickness = (50.0, 5000.0, 100.0)
    parent_rotation, child_rotation = (0.0, 45.0)
    ...
    # Run pybdy
    # process_bdy(setup_filepath=os.path.join("tests", "namelists", "namelist_test_002.bdy"))
